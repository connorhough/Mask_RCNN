...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    x = conv_block(
        x, 3, [
            64, 64, 256], stage=2, block='a', strides=(
    ...
    for i in range(block_count):
        x = identity_block(
            x, 3, [
                256, 256, 1024], stage=4, block=chr(
                98 + i), train_bn=train_bn)
    C4 = x
    # Stage 5
    if stage5:
...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        deltas = utils.batch_slice([deltas, ix], lambda x, y: tf.gather(x, y),
                                   self.config.IMAGES_PER_GPU)
        pre_nms_anchors = utils.batch_slice([anchors, ix], lambda a, x: tf.gather(
            a, x), self.config.IMAGES_PER_GPU, names=["pre_nms_anchors"])
...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    # For each kept ROI, assign a class_id, and for FG ROIs also add bbox
    # refinement.
    rois = rpn_rois[keep]
...
def data_generator(
    ...
    while True:
        try:
            # Increment index to pick next image. Shuffle if at the start of an
            # epoch.
...
class ImageCallback(keras.callbacks.Callback):
    def __init__(self, run, val_dataset, train_dataset):
        ...
        self.image_ids = val_dataset.image_ids[:3]

    def label_image(self, image_id):
        original_image, image_meta, gt_class_id, gt_bbox, gt_mask = load_image_gt(
            self.val_dataset, inference_config, image_id, use_mini_mask=False)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Don't create the thead (stage 5), so we pick the 4th item in the
        # list.
        ...
        if mode == "training":
            ...
            # Losses
            rpn_class_loss = KL.Lambda(lambda x: rpn_class_loss_graph(
                *x), name="rpn_class_loss")([input_rpn_match, rpn_class_logits])
            rpn_bbox_loss = KL.Lambda(lambda x: rpn_bbox_loss_graph(
                config, *x), name="rpn_bbox_loss")([input_rpn_bbox, input_rpn_match, rpn_bbox])
            class_loss = KL.Lambda(lambda x: mrcnn_class_loss_graph(*x), name="mrcnn_class_loss")(
                [target_class_ids, mrcnn_class_logits, active_class_ids])
            bbox_loss = KL.Lambda(lambda x: mrcnn_bbox_loss_graph(
                *x), name="mrcnn_bbox_loss")([target_bbox, target_class_ids, mrcnn_bbox])
            mask_loss = KL.Lambda(lambda x: mrcnn_mask_loss_graph(
                *x), name="mrcnn_mask_loss")([target_mask, target_class_ids, mrcnn_mask])
    ...
    def set_log_dir(self, model_path=None):
        ...
        if model_path:
            ...
            if m:
                ...
                # So, adjust for that then increment by one to start from the
                # next epoch
